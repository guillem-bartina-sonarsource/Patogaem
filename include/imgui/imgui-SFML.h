...
namespace ImGui {
namespace SFML {
IMGUI_SFML_API void Init(uintptr_t windowId, bool hasFocus, const sf::Vector2f& displaySize, bool loadDefaultFont = true);

IMGUI_SFML_API void SetCurrentWindow(uintptr_t windowId);
IMGUI_SFML_API void ProcessEvent(uintptr_t windowId, const sf::Event& event);

IMGUI_SFML_API void Update(uintptr_t windowId, const sf::Vector2i& mousePos, const sf::Vector2f& displaySize, sf::Time dt);

IMGUI_SFML_API void Render(sf::RenderTarget& target);

IMGUI_SFML_API void Shutdown(uintptr_t windowId);
