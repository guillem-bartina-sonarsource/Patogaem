...
struct WindowContext {
    ...
    WindowContext(uintptr_t windowId, bool hasFocus) : windowId(windowId) {
...
void Init(uintptr_t windowId, bool hasFocus, const sf::Vector2f& displaySize, bool loadDefaultFont) {
    ...
    s_windowContexts.push_back(new WindowContext(windowId, hasFocus));
...
void SetCurrentWindow(uintptr_t windowId) {
    for (std::size_t i = 0; i < s_windowContexts.size(); ++i) {
        if (s_windowContexts[i]->windowId == windowId) {
...
void ProcessEvent(uintptr_t windowId, const sf::Event& event) {
...
void Update(uintptr_t windowId, const sf::Vector2i& mousePos, const sf::Vector2f& displaySize, sf::Time dt) {
...
void Shutdown(uintptr_t windowId) {
    // set current context to some window for convenience if needed
    if (windowId == s_currWindowCtx->windowId) {
